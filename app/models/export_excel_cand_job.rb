...
class ExportExcelCandJob
  ...
  # * <tt>:the_candidate_columns</tt> columns
  # * <tt>:dir</tt> temp dir for storage
  #
  def perform(cand_id, sheet, the_candidate_columns, _dir)
    ActiveRecord::Base.connection_pool.with_connection do
      ...
      @sheet_mutex.synchronize do
        sheet.add_row(the_candidate_columns.map do |a_col|
          if CandidateImport.transient_columns.include?(a_col)
            nil
          else
            get_column_value(candidate, a_col, events)
          end
        end)
      end
    end
  ...
  # * <tt>:a_col</tt> String: image being processed
  # * <tt>:dir</tt> String: base file path
  #
  # def certificate_image_column(candidate, a_col, dir)
  #   Rails.logger.info("candidate.baptismal_certificate=#{candidate.baptismal_certificate}")
  #   if a_col.include? 'scanned_certificate'
  ...
  #     end
  #   elsif a_col.include? 'retreat_verification'
  ...
  #       "#{image.filename}:::#{content_type(image.content_type)}:::#{export_filename}"
  #     end
  #   elsif a_col.include? 'scanned_eligibility'
  ...
  #       "#{image.filename}:::#{content_type(image.content_type)}:::#{export_filename}"
  #     end
  #   elsif a_col.include? 'scanned_covenant'
  ...
  # === Returns:
  #
  # * <tt>Object</tt> the value from association if image a_col then true if image is supplied otherwise false
  #
  def get_column_value(candidate, a_col, confirmation_events)
    val = get_column_value_raw(candidate, a_col, confirmation_events)
    return !val.nil? if CandidateImport.image_columns.include?(a_col)

    val unless CandidateImport.image_columns.include?(a_col)
  end

  private

  def get_column_value_raw(candidate, a_col, confirmation_events)
    split = a_col.split('.')
    case split.size
    when 1
      candidate.send(a_col)
    when 2
